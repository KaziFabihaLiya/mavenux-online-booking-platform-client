...
// Error messages are displayed for user feedback
 ...
 const LoginForm = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      await saveOrUpdateUser({
        ...
        console.error("DB save failed, but auth ok:", err);
        toast.error("Partial success - DB sync failed. Try relogin.");
      });
      navigate(from, { replace: true });
      toast.success("Login Successful");
    } catch (err) {
  ...
  if (user) navigate(from);  // Always redirect if user exists
}
  };
